...
def getMetricStructure(midi, signature="2/4", quarterNoteDuration=480.):
    ...
    for note in noteList:
        ...
        if not metric_grouping.has_key(d):
#            print d, totalDuration / note.duration
#            print [0] * int(totalDuration / note.duration)
            # quarter notes are straightforward
#            if d == 1:
#                metric_grouping[d] = [0] * int(totalDuration/note.duration/2)
#            else:
                metric_grouping[d] = [0] * int(totalDuration / note.duration)
    ...
    for note in noteList:        
        total_score = 0
        d = note.duration / quarterNoteDuration
        relevant_structure = metric_grouping[d]
        ...
        if note.duration - quarterNoteDuration < 0.1:
            if (int(floor(position)) % 2 == 1):
                total_score += 1
                metric_grouping[d][position] = 1
            else:
                metric_grouping[d][position] = 0
        ...
        if relevant_structure[position - 1] != 1:
            ...
            upper_levels = [level for level in sorted(metric_grouping.keys()) if level > d]
            for level in upper_levels:
                if metric_grouping[level][int(floor(position * d / level)) - 1] != 1:
                    total_score += 1
                    metric_grouping[level][int(floor(position * d / level))] = 1
        scoreList.append(total_score)
#    print scoreList
    return metric_grouping, scoreList
    
if __name__ == '__main__':
    from tools import midi as mid
    midi = mid.prepare_initial_midi("../../../res/midi_text.txt", "../../../res/sample.midi", 15200)
    track = midi.tracks[0]
    grouping, scores = getMetricStructure(midi)
    print scores
    # maximum beats i.e. number of beats at minimal level
    max_beats = len(grouping[min(grouping.keys())])
    min_level = min(grouping.keys())
    keys = [float(key) for key in grouping.keys()]
    for level in sorted(keys):
        structure = grouping[level]
